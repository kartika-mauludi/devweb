    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title font-weight-bold">Import Account</h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-10">
                        <input type="file" id="excelFileInputAccount" class="form-control" accept=".xls, .xlsx">
                    </div>
                    <div class="col">
                        <a href="{{ asset('template/template_account.xlsx') }}" class="btn btn-sm btn-info">Template</a>
                    </div>
                </div>
                <div class="table-responsive mh-65vh">
                    <table id="importPreviewTableAccount" class="table table-bordered">
                        <thead>
                            <tr>
            ...
            <div class="modal-footer">
                <button id="btnUploadDataAccount" class="btn btn-success" disabled>Upload Data</button>
...
<script>
    $(document).ready(() => {
        let tableImportAccount = $("#importPreviewTableAccount").DataTable({
            paging: false,
            searching: false,
            info: false,
            lengthChange: false,
        ...
        $("#excelFileInputAccount").change(function (e) {
            ...
            reader.onload = function (event) {
                ...
                if (sheet.length > 1) {
                    sheet.slice(1).forEach((row, index) => {
                        let username = row[0] ? row[0].trim() : "";
                        let password = row[1] ? row[1].trim() : "";
                        let errorMessage = "";

                        if (!username) errorMessage += "Judul kosong. ";
                        if (!password) {
                    ...
                    $("#btnUploadDataAccount").prop("disabled", !isValid);
                } else {
                    $("#btnUploadDataAccount").prop("disabled", true);
                }
        ...
        $("#btnUploadDataAccount").click(function () {
            let tableData = [];
            let isValid = true;

            $("#importPreviewTableAccount tbody tr").each(function () {
                let row = $(this).find("td");
                let username = row.eq(1).text().trim();
                let password = row.eq(2).text().trim();
                ...
                tableData.push({ username, password });
            });
            ...
            fetch(`/universities/{{ $university->id }}/accounts/import`, {
                method: "POST",
                headers: {
                ...
                function read() {
                    reader.read().then(({ done, value }) => {
                        if (done) {
                            $("#excelFileInputAccount").val(null);
